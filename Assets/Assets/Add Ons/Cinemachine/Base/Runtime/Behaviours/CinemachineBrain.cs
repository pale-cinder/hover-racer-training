...
namespace Cinemachine
{
    ...
    public class CinemachineBrain : MonoBehaviour
    {
        ...
        /// <summary>
        /// When enabled, orthographic cameras will round their size and position to the nearest pixel
        /// </summary>
        [Tooltip("When enabled, orthographic cameras will round their size and position to the nearest pixel")]
        public bool m_PixelPerfect = false;

        /// <summary>
        /// Relates World dimensions to pixels.  
        /// This should match the PPU of your sprite assets.  
        /// In Pixel-Perfect mode, orthographic size and camera positions will be 
        /// integral multiples of pixels.
        /// </summary>
        [Tooltip("Relates world dimensions to pixels.  This should match the PPU of your sprite assets.  In Pixel-Perfect mode, orthographic size and camera positions will be integral multiples of pixels.")]
        ...
        {
            m_PixelsPerUnit = Mathf.Max(1, m_PixelsPerUnit);
        }
        ...
        void MakePixelPerfect(ref float orthoSize, ref Vector3 camPos)
        {
            if (m_PixelsPerUnit > UnityVectorExtensions.Epsilon 
                && Mathf.Abs(orthoSize) > UnityVectorExtensions.Epsilon)
            {
                float upp = 1f / m_PixelsPerUnit;
                float desiredPix = orthoSize / upp;
                float h = Screen.height / 2;
                if (desiredPix >= h)
                    orthoSize = upp * h * Mathf.Round(desiredPix / h);
                else
                    orthoSize = upp * desiredPix * Mathf.Round(h / desiredPix);

                camPos.x = upp * Mathf.Round(camPos.x / upp);
                camPos.y = upp * Mathf.Round(camPos.y / upp);
                camPos.z = upp * Mathf.Round(camPos.z / upp);
            }
        }
        ...
        private void PushStateToUnityCamera(CameraState state, Camera cam, ICinemachineCamera vcam)
        {
            ...
            Vector3 position = state.FinalPosition;
            float orthoSize = state.Lens.OrthographicSize;
            if (cam.orthographic && m_PixelPerfect)
                MakePixelPerfect(ref orthoSize, ref position);
